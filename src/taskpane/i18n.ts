...
export const I18N = {
  en: {
    ...
    uploadedPartial: (uploaded: number, total: number, failed: string, reason?: string) =>
      `⚠️ Uploaded ${uploaded} of ${total} file(s). Failed: ${failed}${reason ? ` — ${reason}` : ""}`,
  ...
  sv: {
    ...
    uploadedPartial: (uploaded: number, total: number, failed: string, reason?: string) =>
      `⚠️ Laddade upp ${uploaded} av ${total} fil(er). Misslyckades: ${failed}${reason ? ` — ${reason}` : ""}`,
